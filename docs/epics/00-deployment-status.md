...
| E4-T3 | `feature/E4-T3-t4-t5-persistence-queue` (local run) | completed | completed (rsync + compose deploy succeeded; postgres + UI images available) | health-check PASS (`scripts/check_stack.sh`) with stack services and dependency endpoints passing | PR manual (gh missing) |
| E4-T4 | `feature/E4-T3-t4-t5-persistence-queue` (local run) | completed | completed (deploy workflow includes migration precheck hook in `scripts/deploy_sync_health.sh`) | deploy run PASS; migration precheck skipped remotely when script not synced via ignore rules | PR manual (gh missing) |
| E4-T5 | `feature/E4-T3-t4-t5-persistence-queue` (local run) | completed | completed (rsync + compose deploy succeeded with queue adapter code deployed) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
| E5-T1 | `feature/E5-vdb-portkey-orchestration` (local run) | completed | completed (rsync + compose deploy succeeded with runtime provider wiring updates) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
| E5-T2 | `feature/E5-vdb-portkey-orchestration` (local run) | completed | completed (rsync + compose deploy succeeded with Milvus adapter changes) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
| E5-T3 | `feature/E5-vdb-portkey-orchestration` (local run) | completed | completed (rsync + compose deploy succeeded with alternate vector stub path) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
| E5-T4 | `feature/E5-vdb-portkey-orchestration` (local run) | completed | completed (rsync + compose deploy succeeded with Portkey gateway adapter wiring) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
| E5-T5 | `feature/E5-vdb-portkey-orchestration` (local run) | completed | completed (rsync + compose deploy succeeded with secrets/observability hardening) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
| E5-T6 | `feature/E5-vdb-portkey-orchestration` (local run) | completed | completed (rsync + compose deploy succeeded with documentation/runbook updates) | health-check PASS (`scripts/check_stack.sh`) | PR manual (gh missing) |
