...
create_branch() {
  ...
  require_git_repo
  local desc
  desc="$(printf '%s-%s' "$word1" "$word2" | tr '[:upper:]' '[:lower:]')"
  desc="${desc//[^a-z0-9-]/}"
...
deploy_ec2() {
  ...
  ssh "$user_host" "set -euo pipefail; cd '$remote_path'; docker compose -f '$compose_file' pull; docker compose -f '$compose_file' build; docker compose -f '$compose_file' up -d; if [[ -x scripts/migration_bootstrap.sh ]]; then ./scripts/migration_bootstrap.sh; else echo 'migration bootstrap script not found/executable; skipping'; fi"
